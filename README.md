...
- `bin/claude-rc-watchdog` — external watchdog for relay drops. Polls every pane in a tmux session (default `work`), looks at the bottom slice of the pane for claude's status footer (the unique `Model: <provider> <ver> ... | Ctx: <value>` line, NBSP-tolerant) and the `Remote Control active` indicator. The footer wraps across multiple lines in narrow/unattached (e.g. 80-col) panes, so the RC indicator is matched anywhere in that bottom slice rather than on the Model line itself — checking only the Model line treated every healthy narrow pane as RC-missing and SIGTERMed it on a ~2-min loop. When the indicator is missing for `THRESHOLD` consecutive checks it SIGTERMs claude on that pane's tty — `claude-revive` then respawns with `--continue`, re-registering with the relay. After a SIGTERM the pane gets a per-pane cooldown (`CLAUDE_RC_WATCHDOG_COOLDOWN`, default 120s) during which miss-counting is suppressed, so the watchdog doesn't re-kill the session mid-boot before its relay websocket has re-registered (this also avoids the rapid registration churn that makes the relay reject with "Session creation failed"). Also auto-dismisses the "Resume from summary" modal that `--continue` shows on large-context resumes (sends synthetic `Enter` via tmux when both modal option lines appear at the pane bottom and no footer is rendered) so panes don't blackhole on the modal after a SIGTERM. Tunable via `CLAUDE_RC_WATCHDOG_{SESSION,INTERVAL,THRESHOLD,GRACE,COOLDOWN}` env vars; supports `--dry-run` and `--once` for testing. Designed to run as a LaunchAgent (`com.claude-rc-watchdog`) so it survives logout and tmux server restarts.
